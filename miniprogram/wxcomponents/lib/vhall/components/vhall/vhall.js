...
Component({
    ...
    Vhall.VhallMain.on('control', (type, val) => {
      switch (type) {
        ...
        case EVENTE.EVENT_BACKRATE:
          this.setPlayBackRate(this.data.playRateVal); // 设置倍速播放
          break;
      }
    });
  },
  methods: {
    ...
    //倍速播放
    selectPlayRate(e) {
      this.setPlayBackRate(e.target.dataset.type);
      this.setData({
        showPlayRate: false,
        showMain: false,
        playRateVal: e.target.dataset.type
      });
    },
    setPlayBackRate(val) {
    ...
    fullScreenChange: function(e) {
      this.setPlayBackRate(this.data.playRateVal)
      this.setData({
        backShow: false,
