...
Page({
  ...
  onLoad: async function (option) {
    ...
    this.getDetails().then(res => {
      wx.setNavigationBarTitle({
        title: this.data.examPackage.title,
      });
    });
  ...
  getExamQusetionList() {
    ...
    if (this.data.userInfo) {
      examPackageClient.getAppExamObj(parseInt(this.data.packageId), parseInt(this.data.examId)).then((res) => {
        if (res.data) {
          ...
          for (let i in res.data.questionGroups) {
            for (let j in res.data.questionGroups[i].questions) {
              if (res.data.questionGroups[i].name.indexOf('判断') != -1) {
                res.data.questionGroups[i].questions[j].optionList = ["对", "错"];
              }
  ...
  async submit() {
    ...
    if (this.data.userInfo != null && this.data.userInfo.id) {
      ...
      await webClient.sendAsync<any>({
        ...
        if (res.data) {
          wx.navigateTo({
            url: `/examPaper/question/Answerkey?packageId=${this.data.packageId}&examId=${this.data.examId}`
          })
        } else {
          wx.showToast({
            title: "服务器异常，请稍后再试！",
